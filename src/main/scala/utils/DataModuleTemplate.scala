...
(
  ...
  val dataBanks = Seq.tabulate(numBanks)(i => {
    ...
    // delay one clock
    val raddr_dup = if(hasRen) (io.ren.get zip io.raddr).map{case(ren,raddr) => RegEnable(raddr,ren)} else RegNext(io.raddr)
  ...
  for (j <- 0 until numRead) {
    val raddr_dup = if(hasRen) RegEnable(io.raddr(j),io.ren.get(j)) else RegNext(io.raddr(j))
...
class Folded1WDataModuleTemplate[T <: Data](gen: T, numEntries: Int, numRead: Int,
  ...
  val raddr = if (isSync) {
    if(hasRen) (io.raddr zip io.ren.get) map {case(raddr,ren) => RegEnable(raddr, ren)}
