...
class LoadUnit_S2(implicit p: Parameters) extends XSModule with HasLoadHelper with HasCircularQueuePtrHelper with HasDCacheParameters {
  ...
  io.loadDataFromDcache.forward_refillBuffer := RegEnable(io.forward_refillBuffer, io.in.valid)
  io.loadDataFromDcache.forwardData_refillBuffer := RegEnable(io.forwardData_refillBuffer, io.in.valid)
