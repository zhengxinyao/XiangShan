...
class VluopBundle(implicit p: Parameters) extends XSBundle {
  ...
  def apply(uop: VecOperand, is_pre: Bool = false.B, is_allo: Bool = false.B) = {
    when (is_pre) {
...
class VluopQueue(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper
{
  ...
  for (i <- 0 until exuParameters.LduCnt) {
    //already_in(i) := Cat(VluopEntry.map(_.rob_idx === io.loadRegIn(i).bits.uop.robIdx.value)).orR
    already_in(i) := Cat((0 until VlUopSize).map(j => VluopEntry(j).rob_idx === io.loadRegIn(i).bits.uop.robIdx.value && io.loadRegIn(i).valid && pre_allocated(j))).orR
  ...
  when (enq_valid(0) || enq_valid(1)) {
    when(enq_valid(0) && enq_valid(1) &&
      ...
      enqPtr.value := enqPtr.value + io.loadRegIn(0).bits.total_num
      for (i <- 0 until 8) {
        when (i.U < io.loadRegIn(0).bits.total_num) {
          ...
          VluopEntry(enqPtr.value + i.U).apply(uop = inUop, is_pre = isPer)
      ...
      for (i <- 0 until 8) {
        when (i.U < io.loadRegIn(1).bits.total_num) {
          ...
          VluopEntry(enqPtr.value + io.loadRegIn(0).bits.total_num + i.U).apply(uop = inUop, is_pre = isPer)
      ...
      enqPtr.value := enqPtr.value + io.loadRegIn(1).bits.total_num
      for (i <- 0 until 8) {
        when (i.U < io.loadRegIn(1).bits.total_num) {
          val inUop = WireInit(io.loadRegIn(1).bits)
          val isPer = !(i.U === io.loadRegIn(1).bits.inner_idx)
    ...
    when (!enq_valid(0) && io.loadRegIn(0).fire) {
      ...
      for (i <- 0 until VlUopSize) {
        when (VluopEntry(i).rob_idx === io.loadRegIn(0).bits.uop.robIdx.value && valid(i) && pre_allocated(i)) {
          VluopEntry(i).apply(uop = io.loadRegIn(0).bits, is_allo = true.B)
          allocated(i)     := true.B
          debug_hit(i)     := true.B
        }
    ...
    when (!enq_valid(1) && io.loadRegIn(1).fire) {
      ...
      for (i <- 0 until VlUopSize) {
        when (VluopEntry(i).rob_idx === io.loadRegIn(1).bits.uop.robIdx.value && valid(i) && pre_allocated(i)) {
          VluopEntry(i).apply(uop = io.loadRegIn(1).bits, is_allo = true.B)
          allocated(i)     := true.B
          debug_hit(i)     := true.B
        }
        assert(PopCount(debug_hit) <= 1.U, "VluopQueue Multi-Hit!")
