...
class LFST(implicit p: Parameters) extends XSModule {
  ...
  (0 until exuParameters.StuCnt).map(i => {
    ...
    (0 until LFSTWidth).map(j => {
      when(io.storeIssue(i).valid && io.storeIssue(i).bits.uop.cf.storeSetHit && io.storeIssue(i).bits.uop.robIdx === robIdxVec(io.storeIssue(i).bits.uop.cf.ssid)(j)){
  ...
  when(RegNext(io.redirect.fire())) {
    (0 until LFSTSize).map(i => {
      (0 until LFSTWidth).map(j => {
        ...
        val last_check_position = WireInit(allocPtr(i) + j.U)
        when(!validVec(i)(check_position) && validVec(i)(last_check_position)){
