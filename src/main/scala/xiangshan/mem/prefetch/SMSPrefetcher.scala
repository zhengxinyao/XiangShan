...
class ActiveGenerationTable()(implicit p: Parameters) extends XSModule with HasSMSModuleHelper {
  ...
  val s0_alloc = !s0_region_hit && !s0_match_prev
  ...
  val s1_alloc = s1_cross_region_match
  ...
  val s1_pf_gen_valid = prev_lookup_valid && (s1_alloc || s1_update) && Mux(s1_pf_gen_decr_mode,
...
class SMSPrefetcher()(implicit p: Parameters) extends BasePrefecher with HasSMSModuleHelper {
  ...
  val pf_gen_req = Mux(agt_gen_valid,
