  ...
  with HasExceptionNO
{
  ...
  io.backend.fromFtq <> DontCare//ftq.io.toBackend
  io.frontendInfo.bpuInfo <> DontCare//ftq.io.bpuInfo
  ...
  if(print_perfcounter){
    ...
    // val ftq_perf     = DontCare//ftq.perfEvents.map(_._1).zip(ftq.perfinfo.perfEvents.perf_events)
    // val bpu_perf     = DontCare//bpu.perfEvents.map(_._1).zip(bpu.perfinfo.perfEvents.perf_events)
