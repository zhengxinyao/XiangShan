...
class OracleBP(implicit p: Parameters) extends BasePredictor with OracleBPParams with OracleBPPredictWidth {
  ...
  * */
  ...
  if (OracleBranch) {
    for (i <- 0 until numBr) {
      io.out.s3.full_pred.br_taken_mask(i) := RegEnable(io.out.s2.full_pred.br_taken_mask(i), io.s2_fire)
      io.out.s3.full_pred.targets(i) := RegEnable(io.out.s2.full_pred.targets(i), io.s2_fire)
    }
  }
  if (OracleIndirect) {
    io.out.s3.full_pred.targets.last := RegEnable(io.out.s2.full_pred.targets.last, io.s2_fire)
  }

  io.out.s3.brIdx := RegEnable(io.out.s2.brIdx, io.s2_fire)
  io.out.s2.brIdx := brIdx
  ...
  if (OracleBranch) {
    for (i <- 0 until numBr) {
      io.out.s2.full_pred.br_taken_mask(i) := 0.U
    }
    when (pc_hit_br) {
      io.out.s2.full_pred.br_taken_mask(0) := takens(pc_hit_br_pos - 1.U)
      if (OracleBranchTarget) {
        io.out.s2.full_pred.targets(0) := targets(pc_hit_br_pos - 1.U)
      }
    }
  }
  when (pc_hit_jr) {
    if (OracleBranch)
      io.out.s2.full_pred.br_taken_mask(1) := takens(pc_hit_jr_pos - 1.U) && !types(pc_hit_jr_pos - 1.U)
    if (OracleIndirect && OracleIndirectTarget) {
      when (types(pc_hit_jr_pos - 1.U)) {
        io.out.s2.full_pred.targets(1) := targets(pc_hit_jr_pos - 1.U)
      }
    }
  ...
  io.out.last_stage_meta := RegEnable(RegEnable(resp_meta.asUInt, io.s1_fire), io.s2_fire)
  generatePerfEvent()
}