...
class Tage(implicit p: Parameters) extends BaseTage {
  ...
  for (w <- 0 until TageBanks) {
    for (i <- 0 until TageNTables) {
      ...
      tables(i).io.update.takens(w)  := RegEnable(updateTakens(w)(i), enable = realWen)
      tables(i).io.update.alloc(w)   := RegEnable(updateAlloc(w)(i), enable = realWen)
      tables(i).io.update.oldCtrs(w) := RegEnable(updateOldCtrs(w)(i), enable = realWen)

      tables(i).io.update.uMask(w)   := RegEnable(updateUMask(w)(i), enable = realWen)
      tables(i).io.update.us(w)      := RegEnable(updateU(w)(i), enable = realWen)
      tables(i).io.update.reset_u(w) := RegEnable(updateResetU(w), enable = realWen)
      // use fetch pc instead of instruction pc
      tables(i).io.update.pc       := RegEnable(update.pc, enable = realWen)
      tables(i).io.update.folded_hist := RegEnable(updateFHist, enable = realWen)
      tables(i).io.update.ghist := RegEnable(io.update.bits.ghist, enable = realWen)
    }
  }
