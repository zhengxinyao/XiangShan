...
class ITTage(implicit p: Parameters) extends BaseITTage {
  ...
  val s2_allocLFSR   = LFSR64(io.s1_fire.head)(ITTageNTables - 1, 0)
  ...
  for (i <- 0 until ITTageNTables) {
    ...
    tables(i).io.update.correct := RegEnable(updateCorrect(i), enable = updateMask(i))
    tables(i).io.update.target := RegEnable(updateTarget(i), enable = updateMask(i))
    tables(i).io.update.old_target := RegEnable(updateOldTarget(i), enable = updateMask(i))
    tables(i).io.update.alloc := RegEnable(updateAlloc(i), enable = updateMask(i))
    tables(i).io.update.oldCtr := RegEnable(updateOldCtr(i), enable = updateMask(i))

    tables(i).io.update.reset_u := RegEnable(updateResetU, enable = updateMask(i))
    tables(i).io.update.uValid := RegEnable(updateUMask(i), enable = updateMask(i))
    tables(i).io.update.u := RegEnable(updateU(i), enable = updateMask(i))
    tables(i).io.update.pc := RegEnable(update.pc, enable = updateMask(i))
    // use fetch pc instead of instruction pc
    tables(i).io.update.folded_hist := RegEnable(updateFhist, enable = updateMask(i))
  }
