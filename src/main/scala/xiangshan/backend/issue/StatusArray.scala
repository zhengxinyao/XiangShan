...
class StatusArray(params: RSParams)(implicit p: Parameters) extends XSModule
  ...
  (statusArrayValidNext zip replayArrayNext).foreach { case (statusValid, replay) => when(statusValid === 0.B) { replay := RSFeedbackType.feedbackInvalid } }
  io.rsFeedback := VecInit((0 until 5).map(index => statusArrayValid.zip(replayArray).map {
    case (statusValid, replay) => statusValid && replay === index.U
  }.reduce(_ || _)))
