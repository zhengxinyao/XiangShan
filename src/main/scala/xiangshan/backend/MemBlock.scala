  ...
  with HasPerfEvents
{
  ...
  if (refillBothTlb) {
    ...
    dtlb_ld.map(_.ptw.resp.valid := ptw_resp_v && Cat(ptw_resp_next.vector.take(exuParameters.LduCnt + coreParams.L1DPrefetchPipelineWidth)).orR)
    dtlb_st.map(_.ptw.resp.valid := ptw_resp_v && Cat(ptw_resp_next.vector.drop(exuParameters.LduCnt + coreParams.L1DPrefetchPipelineWidth)).orR)
  }
