...
class Rename(implicit p: Parameters) extends XSModule {
  ...
  if (EnableIntMoveElim) {
    ...
    XSPerfAccumulate("move_elim_enabled", PopCount(Seq.tabulate(RenameWidth)(i => io.out(i).fire() && io.out(i).bits.eliminatedMove)))
    XSPerfAccumulate("move_elim_cancelled", PopCount(Seq.tabulate(RenameWidth)(i => io.out(i).fire() && io.in(i).bits.ctrl.isMove && !io.out(i).bits.eliminatedMove)))
    // causes
    XSPerfAccumulate("move_elim_cancelled_psrc_bypass", PopCount(Seq.tabulate(RenameWidth)(i => io.out(i).fire() && io.in(i).bits.ctrl.isMove && !io.out(i).bits.eliminatedMove && { if (i == 0) false.B else io.renameBypass.lsrc1_bypass(i-1).orR })))
    XSPerfAccumulate("move_elim_cancelled_cnt_limit", PopCount(Seq.tabulate(RenameWidth)(i => io.out(i).fire() && io.in(i).bits.ctrl.isMove && !io.out(i).bits.eliminatedMove && isMax.get(io.out(i).bits.psrc(0)))))
    XSPerfAccumulate("move_elim_cancelled_inc_more_than_one", PopCount(Seq.tabulate(RenameWidth)(i => io.out(i).fire() && io.in(i).bits.ctrl.isMove && !io.out(i).bits.eliminatedMove && { if (i == 0) false.B else psrc_cmp(i-1).orR })))
